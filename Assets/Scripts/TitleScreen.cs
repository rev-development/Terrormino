...
public class TitleScreen : MonoBehaviour
{
	...
	public void Start()
	{
		_skinnedMeshRenderers = gameObject.TryFindComponentsInChildren<SkinnedMeshRenderer>();
		_meshRenderers = gameObject.TryFindComponentsInChildren<MeshRenderer>();
	...
	public void Update()
	{
		if (_beginTransition)
		{
			if (_isDissolving) DissolveConsole();
	...
	public void BeginGame() => SceneManager.LoadScene(_sceneName);
	...
	public void DissolveConsole()
	{
		...
		foreach (var meshRenderer in _meshRenderers) meshRenderer.material.SetFloat("_DissolveValue", _dissolveValue);

		if (_dissolveValue >= 1) _isDissolving = false;
	}
}