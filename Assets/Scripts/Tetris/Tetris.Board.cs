...
namespace Tetris
{
	...
	public class Board : MonoBehaviour
	{
		...
		public void ClearLines()
		{
			...
			var row = bounds.yMin;
			var linesCleared = 0; // Track how many rows we clear this piece
			...
			while (row < bounds.yMax)
			{
				...
				if (IsLineFull(row))
				{
					...
					for (var shiftRow = row; shiftRow < bounds.yMax; shiftRow++)
					{
						for (var col = bounds.xMin; col < bounds.xMax; col++)
						{
...
#region Sub-Functions
		...
		public bool IsValidPosition(Vector3Int[] cells, Vector3Int position)
		{
			...
			for (var i = 0; i < cells.Length; i++)
			{
				...
				if (!BoardBounds.Contains(new Vector2Int(tilePosition.x, tilePosition.y))) return false;

				if (BoardTilemap.HasTile(tilePosition)) return false;
			}
...
#region Lifecycle
		...
		public void Awake()
		{
			...
			for (var i = 0; i < Tetrominoes.Length; i++) Tetrominoes[i].Initialize();
		}

		public void Start()
		{
			BoardTilemap = TryFind.ComponentOnGameObjectByName<Tilemap>("BoardTilemap");
			ActivePiece = gameObject.TryFindComponent<ActivePieceController>();
...
#region Config Values
		...
		private ITetrisConfig _tetrisConfig = new TetrisConfig
											  {
												  TetrisGravityDelay = 1f,
												  TetrisMoveDelay = 0.1f,
												  TetrisLockDelay = 0.5f,
											  };
