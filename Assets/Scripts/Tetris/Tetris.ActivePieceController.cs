...
#region Input Listeners
		...
		{
			var rotateInput = Helpers.Math.RoundNearestNonZeroInt(inputAction.ReadValue<float>());
...
#region Subfunctions
		...
		{
			var wallKickIndex = Helpers.Math.Wrap(
				(rotateInput + RotationIndex) * 2 - (rotateInput < 0 ? 1 : 0),
				0,
				Shape.WallKicks.GetLength(0)
			);

			for (var i = 0; i < Shape.WallKicks.GetLength(1); i++)
			{
				var wallKickMoveInput = Shape.WallKicks[wallKickIndex, i];

				if (TryMove(wallKickMoveInput, cells) != null) return TryMove(wallKickMoveInput, cells);
			}
		...
		private Vector3Int[] GenerateRotationCells(int rotateInput)
		{
			...
			var matrix = ShapeVecs.RotationMatrix;
...
#region Lifecycle
		...
		public void Start()
		{
			Helpers.Ext.GameObjectExt.CheckIfSetInInspector(gameObject, Board, "Board");
