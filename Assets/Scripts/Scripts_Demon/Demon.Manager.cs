...
namespace Demon
{
    public class Manager : MonoBehaviour
    {
        ...
        {
            _nightSpeed = nightConfig.DemonSpeed;
            _nightPatrolDuration = nightConfig.DemonPatrolDuration;
            _nightFreezeDuration = nightConfig.DemonFreezeDuration;
        ...
        public void SpawnDemon()
        {
            if (Demons.Count == 0)
            {
                ...
                if (DemonPrefab.TryGetComponent(out Collider demonCollider))
                {
                    ...
                    Vector3 spawnLocation = new(Random.Range(spawnBounds.min.x, spawnBounds.max.x), spawnBounds.center.y, Random.Range(spawnBounds.min.z, spawnBounds.max.z));
        ...
        public interface IDemonConfig
        {

            float DemonSpeed { get; set; }
            float DemonPatrolDuration { get; set; }
            float DemonFreezeDuration { get; set; }
