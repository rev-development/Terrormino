...
namespace Demon
{
	public class LightFear : MonoBehaviour
	{
		...
		public void OnEnable() => Banish.AddListener(StartDelayedDestroy);

		public void LateUpdate()
		{
			if (_destroyInLateUpdate) Destroy(gameObject);
		}

		private void OnTriggerExit(Collider other)
		{
			if (other.CompareTag("Flashlight")) Illuminate.Invoke(false);
		}
		...
		private void OnTriggerStay(Collider other)
		{
			if (other.CompareTag("Flashlight"))
			{
				...
				if (shake.IsActive)
				{
					...
					if (Health.Value <= 0) Banish.Invoke(gameObject);
				}
				else
				{
		...
		public void OnDisable() => Banish.RemoveAllListeners();

		public void StartDelayedDestroy(GameObject _) => _destroyInLateUpdate = true;
	}
}