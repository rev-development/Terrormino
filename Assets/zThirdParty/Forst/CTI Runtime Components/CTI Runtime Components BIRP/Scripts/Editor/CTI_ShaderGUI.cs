...
namespace ThirdParty.Forst.CTI_Runtime_Components.CTI_Runtime_Components_BIRP.Scripts.Editor
{
    public class CTI_ShaderGUI : ShaderGUI
    {
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            EditorGUILayout.HelpBox(
                    "Please note that params marked with '*' must be synced across bark, leaf and maybe even the billboard material.",
                    MessageType.Info
                );
            ...
            else if (targetMat.HasProperty("_BumpSpecAOMapArray"))
            {
                ...
                {
                    targetMat.SetTexture(
                            "_BumpSpecAOMapArray",
                            Resources.Load("CTI_default_normal_spec_ao_Texture2DArray") as Texture2DArray
                        );
                }
            }
            ...
            if (targetMat.HasProperty("_DetailNormalMapX"))
            {
                if (targetMat.GetTexture("_DetailNormalMapX") == null)
                {
                    targetMat.SetTexture(
                            "_DetailNormalMapX",
                            Resources.Load("CTI_default_normal_spec_ao") as Texture2D
                        );
                }
            }
            ...
            if (targetMat.HasProperty("_TranslucencyMap"))
            {
                if (targetMat.GetTexture("_TranslucencyMap") == null)
                {
                    targetMat.SetTexture(
                            "_TranslucencyMap",
                            Resources.Load("CTI_default_ao_trans_smoothness") as Texture2D
                        );
                }
            }
            ...
            if (targetMat.HasProperty("_AverageCol"))
            {
                ...
                {
                    if (targetMat.HasProperty("_MainTex")
                        && targetMat.GetTexture("_MainTex") != null)
                    {
                        getAverageColor(targetMat.GetTexture("_MainTex") as Texture2D);
                    }
                    else if (targetMat.HasProperty("_MainTexArray")
                             && targetMat.GetTexture("_MainTexArray") != null)
                    {
        ...
        private void getAverageColor(Texture2D sourceTex)
        {
            ...
            if (sourceTex)
            {
                ...
                // is array
                var aColor = sourceTex.GetPixels(
                        0,
                        0,
                        1,
                        1,
