...
)
{
    ...
    //#else
    float4 vOscillations = SmoothTriangleWave(float4(TreeWorldPos.x + sinuswave, TreeWorldPos.z + sinuswave * 0.7,
                                                     TreeWorldPos.x + shiftedsinuswave,
                                                     TreeWorldPos.z + shiftedsinuswave * 0.8));
    //#endif
    ...
    if (IsLeaves)
    {
        ...
        float offset = fracs.x + fracs.y + fracs.z /* this adds a lot of noise, so we use * 0.1 */ + (BranchBending +
            Phase) * leafNoise;
        float tFrequency = _TumbleFrequency * (timeParams.x + fObjPhase * 10.0);
        float4 vWaves1 = SmoothTriangleWave(float4((tFrequency + offset) * (1.0 + offset * 0.25),
                                                   tFrequency * 0.75 + offset, tFrequency * 0.5 + offset,
                                                   tFrequency * 1.5 + offset));
        ...
        {
            float lodfade = (VertexColor.b > (1.0f / 255.0f * 126.0f)) ? 1 : 0;
            // Make sure that the 1st vertex is taken into account
        ...
        {
            o_positionOS.xyz += o_normalOS.xyz * SmoothTriangleWave(
                    tumbleInfluence * timeParams.x * AdvancedEdgeBending.y + Phase) * AdvancedEdgeBending.x * Flutter *
                absWindStrength;
        }
    }
    ...
    o_positionOS.xyz += ((vWavesSum.xyx * bend) + (Wind.xyz * vWavesSum.y * animParams.w)) * absWindStrength *
        _CTI_SRP_Turbulence;
    ...
    o_colorVariationAmbient.x = saturate(
        (frac(TreeWorldPos.x + TreeWorldPos.y + TreeWorldPos.z) + frac(
            (TreeWorldPos.x + TreeWorldPos.y + TreeWorldPos.z) * 3.3)) * 0.5);
